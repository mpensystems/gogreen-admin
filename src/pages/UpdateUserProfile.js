...
import { getUserByID, updateUserProfile } from "../api/adminApis";
...
export default () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await getUserByID(token, aid);
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      const response = await updateUserProfile(formData, token, aid);
