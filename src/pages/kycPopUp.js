...
const KycPopUp = ({ token, rid, show, handleClose, modalData = {}, title }) => {
  ...
  return (
    <Modal show={show} onHide={handleClose} size="lg" centered>
      ...
      <Modal.Body className="d-flex flex-column flex-lg-row">
        {/* Left side: Images */}
        <div className="d-flex flex-column align-items-center" style={{ flex: "1", marginBottom: "20px", maxWidth: "100%", width: "100%" }}>
          <div style={{ marginBottom: "20px", width: "100%", maxHeight: "300px" }}>
            <h6>{title === "Pan Card Details" ? " " : title === "Bank Details" ? "Cancelled Cheque" : "Front Side"}</h6>
            {frontImageUrl ? (
              <ImageComponent token={token} rid={rid} fileid={frontImageUrl} />
            ) : (
        ...
        {/* Right side: Information */}
        <div className="d-flex justify-content-center align-items-start flex-column" style={{ flex: "1", padding: "20px", textAlign: "left", overflowY: "auto", maxWidth: "100%", width: "100%" }}>
          {Object.keys(modalData).length > 0 ? (
            Object.entries(modalData)
              .filter(([key]) => !["frontImageUrl", "backImageUrl", "title"].includes(key))
              .map(([key, value]) => (
                <div key={key} style={{ display: "flex", marginBottom: "10px" }}>
                  <div style={{ fontWeight: "bold", width: "150px" }}>{labelMapping[key]}:</div>
                  <div style={{ flex: 1 }}>{value}</div>
